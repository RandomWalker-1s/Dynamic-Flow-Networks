...
def build_corridor():
    ...
    source_1 = dfn.cell.Source(
        ID = 'source_1',
        initial_condition={'density': [0]*state_len},
        boundary_inflow = dfn.flow.BoundaryInflow([1250]*state_len),
        sending = dfn.flow.BufferSendingFlow([1250]*state_len, ignore_queue=True),
    )
...
plt.title('Mainline flow: 4800, on-ramp flow: 1250')

plt.grid()

plt.savefig('./ACTM_example_3.pdf')
plt.close()