...
def build_corridor():
    ...
    source_0 = dfn.cell.Source(
        ...
        boundary_inflow = dfn.flow.BoundaryInflow([4750]*state_len),
        sending = dfn.flow.BufferSendingFlow([4750]*state_len, ignore_queue=True),
    )
...
plt.scatter(corridor.link_list[1].state_output['density'][200, :], corridor.link_list[0].state_output['density'][200, :], s=5)
plt.scatter(corridor.link_list[1].state_output['density'][3600, :], corridor.link_list[0].state_output['density'][3600, :], s=10, color='k')

plt.legend(['12s', '60s', '300s', '600s', '1200s', '21600s'], loc='upper right')
...
plt.title('Mainline flow: 4750, on-ramp flow: 1200')

plt.grid()

plt.savefig('./ACTM_example_2.pdf')
plt.close()