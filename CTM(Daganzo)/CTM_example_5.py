...
def build_corridor():
    ...
    for i in range(num_link):
        initial_density = 70 if i <= 50 else 250
...
plt.tight_layout()
plt.savefig('./Corridor_example_5_fig_1.pdf')
plt.close()


plt.figure(figsize=(6, 5))
plt.plot(range(21), np.concatenate([[0], np.cumsum(link_density[0, 36:56])]), 'o--')
plt.plot(range(21), np.concatenate([[0], np.cumsum(link_density[8, 36:56])]), 'o--')
plt.plot(range(21), np.concatenate([[0], np.cumsum(link_density[16, 36:56])]), 'o--')
plt.plot(range(21), np.concatenate([[0], np.cumsum(link_density[24, 36:56])]), 'o--')
plt.plot(range(21), np.concatenate([[0], np.cumsum(link_density[32, 36:56])]), 'o--')
...
plt.grid()
plt.tight_layout()
plt.savefig('./Corridor_example_5_fig_2.pdf')
plt.close()